  ...
  const createRoom = (e) => {
    if(roomTitle ===''){
     ...
     if(roomStatus){
      ...
      console.log("방정보",roomTitle,roomPassword,roomPurpose,round,studyTime,openedAt );
     }
    ...
    { showModal ?
      ...
      <ModalBox>
        <ModalInnerContainer>
         ...
         <div className="group_modal_relative">
          <Input createGroup value={roomTitle} _onChange={is_filled} className="mb20" text="방제" placeholder="공부 목적을 적어주세요" margin="16px 0 0 0"/>
          <div className="group_modal_roomStatus" onClick={is_status}> 
            {roomStatus ? <img src={unlock} alt="공개방" />: <img src={lock} alt="비공개방" />} 
          </div>
          {roomStatus ? null: (<Input createGroup value={roomPassword} _onChange={(e)=>{setRoomPassword(e.target.value)}} className="mb20" text="비밀번호" placeholder="비밀방 비밀번호 " margin="16px 0 0 0"/>)} 
          
         </div>
          
          <Select className="mb20" createGroup text="공부 목적" value={roomPurpose} name="purpose"
           _onChange={(e)=>{  const selectedPurpose = e.target.value;  setRoomPurpose(selectedPurpose); }}>
          ...
          <Select className="mb20" createGroup text="라운드" value={round} name="round"
           _onChange={(e)=>{  const selectedRound = e.target.value;  setRound(selectedRound); }}>
          ...
          <Input checkbox _name="studyTime" _onChange={is_study} value="25" text="25분/5분" />
          <Input checkbox _name="studyTime" _onChange={is_study} value="50" text="50분/10분"/>

          <p className="group_modal_subtitle">시작시간</p>
          <Input checkbox _name="startTime" _onChange={is_start} value="0" text="바로 시작" />
          <Input checkbox _name="startTime" _onChange={is_start} value="3" text="3분 뒤 시작"/>
          <Input checkbox _name="startTime" _onChange={is_start} value="5" text="5분 뒤 시작"/>
          

          <Button _onClick={createRoom} border="none" height="54px" radius="11px" fontSize="18px"  margin="40px 0 40px 0" weight="600" >
            생성하기
          </Button>
